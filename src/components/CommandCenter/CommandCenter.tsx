...
const CommandCenter: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === 'Enter' && results[selectedIndex]) { e.preventDefault(); saveRecent(query); onClose(); }
    };
    window.addEventListener('keydown', h);
    return () => window.removeEventListener('keydown', h);
  }, [isOpen, onClose, results, selectedIndex, query, saveRecent]);
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh]">
      ...
      <div ref={containerRef} className="relative w-full max-w-2xl mx-4 card bg-navy-light border border-electric/20 shadow-2xl animate-fade-in">
        ...
        <div className="max-h-96 overflow-y-auto">
          {!query && recentSearches.length > 0 && <div className="p-4"><p className="text-xs text-slate uppercase tracking-wider mb-3">Recent Searches</p>{recentSearches.slice(0, 5).map((s, i) => <button key={i} onClick={() => setQuery(s.query)} className="flex items-center gap-3 w-full p-2 rounded-lg hover:bg-navy-lightest text-left"><Clock className="w-4 h-4 text-slate" /><span className="text-sm text-slate-light">{s.query}</span></button>)}</div>}
          {query && results.map((r, i) => (
            <button key={r.id} onClick={() => { saveRecent(query); onClose(); }} className={`flex items-center gap-3 w-full p-3 rounded-lg text-left ${i === selectedIndex ? 'bg-electric/10 border border-electric/20' : 'hover:bg-navy-lightest'}`}>
              <div className="w-8 h-8 rounded-lg bg-electric/10 flex items-center justify-center"><User className="w-4 h-4 text-electric" /></div>
