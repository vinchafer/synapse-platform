        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.map(p => (
            <div key={p.id} onClick={() => setSelected(p)} className={`p-4 border-b border-navy-lightest cursor-pointer transition-colors ${selected?.id === p.id ? 'bg-electric/10' : 'hover:bg-navy-lightest/50'}`}>
              ...
              <div className="flex gap-3 text-xs text-slate"><span><Lightbulb className="w-3 h-3 inline" /> {p.lessonsLearned}</span><span><FileText className="w-3 h-3 inline" /> {p.decisions}</span></div>
            </div>
          ))}
        </div>
      </div>
        ...
        {selected ? (
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="card p-6"><Lightbulb className="w-5 h-5 text-electric mb-2" /><h3 className="text-lg font-bold text-white">Lessons Learned</h3><p className="text-4xl font-bold text-electric mt-2">{selected.lessonsLearned}</p></div>
              <div className="card p-6"><FileText className="w-5 h-5 text-electric mb-2" /><h3 className="text-lg font-bold text-white">Decisions</h3><p className="text-4xl font-bold text-electric mt-2">{selected.decisions}</p></div>
            </div>
          </div>
